...
// Mobile rewind.ts imports only the api singleton and the chat store (no
// sync-poll baseline like desktop), so those are the only modules mocked.
...
describe('rewindToMessage', () => {
  it('rewinds by graph id and truncates the local transcript', async () => {
