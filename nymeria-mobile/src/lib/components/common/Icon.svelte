<script lang="ts">
  ...
  /**
   * Inline icon set, redrawn from Lucide (https://lucide.dev).
   *
   * Every entry stores the *inner* SVG markup (paths / circles / rects)
   * for an icon on Lucide's 24×24 grid. The outer <svg> below sets the
   * common Lucide attributes (`fill="none"`, `stroke="currentColor"`,
   * `stroke-width="2"`, `stroke-linecap="round"`, `stroke-linejoin="round"`)
   * so each entry stays minimal and they all render with a coherent
   * 2px-stroke, rounded-cap visual.
   *
   * The key names match the previous icon set exactly so no caller needs
   * to be updated — only the visual changes (Material-Design filled →
   * Lucide stroke).
   */
  const icons: Record<string, string> = {
    ...
    arrowUp: '<path d="m5 12l7-7l7 7m-7 7V5"/>',
    ...
    chevronRight: '<path d="m9 18l6-6l-6-6"/>',
    chevronLeft: '<path d="m15 18l-6-6l6-6"/>',
    ...
    // Restored to the pre-overhaul filled "terminal chevron" — fill/stroke
    // overrides flip this single glyph from Lucide's stroke-style to the
    // original Material-Design filled look.
    terminal: '<path d="M8 5l8 7-8 7v-3l5-4-5-4V5z" fill="currentColor" stroke="none"/>',
    ...
    externalLink: '<path d="M21 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/>',
    globe: '<circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/>',
    success: '<circle cx="12" cy="12" r="10"/><path d="m9 12l2 2l4-4"/>',
    menu: '<path d="M4 5h16M4 12h16M4 19h16"/>',
    moreVertical: '<circle cx="12" cy="12" r="1"/><circle cx="12" cy="5" r="1"/><circle cx="12" cy="19" r="1"/>',
    // Lucide's loader-circle is a 3/4 arc — combined with the .spinning
    // animation below it reads as a classic spinner.
    ...
    bellOff: '<path d="M10.268 21a2 2 0 0 0 3.464 0M17 17H4a1 1 0 0 1-.74-1.673C4.59 13.956 6 12.499 6 8a6 6 0 0 1 .258-1.742M2 2l20 20M8.668 3.01A6 6 0 0 1 18 8c0 2.687.77 4.653 1.707 6.05"/>',
    // Crescent moon (Lucide). Used as the calm "nothing pending" empty-state mark.
    ...
    refresh: '<path d="M3 12a9 9 0 0 1 9-9a9.75 9.75 0 0 1 6.74 2.74L21 8"/><path d="M21 3v5h-5m5 4a9 9 0 0 1-9 9a9.75 9.75 0 0 1-6.74-2.74L3 16"/><path d="M8 16H3v5"/>',
    rewind: '<path d="M3 12a9 9 0 1 0 9-9a9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/>',
    ...
    textEdit: '<path d="M13 21h8m.174-14.188a1 1 0 0 0-3.986-3.987L3.842 16.174a2 2 0 0 0-.5.83l-1.321 4.352a.5.5 0 0 0 .623.622l4.353-1.32a2 2 0 0 0 .83-.497z"/>',
    ...
    // Cog uses the same Lucide settings glyph as `settings` — they read as
    // the same affordance in this app, so we ship one shape for both keys.
    cog: '<path d="M9.671 4.136a2.34 2.34 0 0 1 4.659 0a2.34 2.34 0 0 0 3.319 1.915a2.34 2.34 0 0 1 2.33 4.033a2.34 2.34 0 0 0 0 3.831a2.34 2.34 0 0 1-2.33 4.033a2.34 2.34 0 0 0-3.319 1.915a2.34 2.34 0 0 1-4.659 0a2.34 2.34 0 0 0-3.32-1.915a2.34 2.34 0 0 1-2.33-4.033a2.34 2.34 0 0 0 0-3.831A2.34 2.34 0 0 1 6.35 6.051a2.34 2.34 0 0 0 3.319-1.915"/><circle cx="12" cy="12" r="3"/>',
