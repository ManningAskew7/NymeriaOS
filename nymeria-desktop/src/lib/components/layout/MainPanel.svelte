<script lang="ts">
  ...
  // kickoff in one round-trip. The root set comes from the shared commands
  // store (one catalog fetch also serving the palette), which degrades to a
  // static fallback on a failed fetch.
