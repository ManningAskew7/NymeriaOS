...
//
// Not to be confused with chatStore.rewindLastAssistantToStablePoint(), the
// provider-retry stream cleanup.
 ...
 * between render and click); callers should refresh and surface an error.
 */
export async function resolveRewindTarget(
  ...
  for (let i = fresh.messages.length - 1; i >= 0; i--) {
    ...
    if (seen === k) {
      ...
      // message not yet polled in), which would silently shift the cut point.
      // Only trust the match when the authoritative message actually contains
      // the target bubble's text; the stored copy may carry server-side
      // prefixes (time context), so containment rather than equality. A false
      // negative falls back to the stale_target reload, after which bubbles
      // are hydrated with graph ids and the fast path applies.
 ...
 * transcript is truncated and the sync-poll baseline refreshed so the poller
 * does not double-reload. On a stale target (404) the transcript is
...
async function reloadHistoryAfterRewindMismatch(threadId: string): Promise<void> {
  try {
    ...
    // Leave the transcript as-is; the sync poller reconciles on its next tick.
  }
}
