...
// Live-attach request (backlog #87): navigation/syncPoll ask the chat panel
// to watch a holder turn this client did not start. `seq` makes each request
...
// server-wide RAG engine config in ServerSettings: these are each account's own.
...
export interface ServerSettings {
  ...
  // Read-only effective tier refs (provider:model or bare model) the
  // fast/smart/background aliases resolve to, used by the per-thread tier quick-pick.
