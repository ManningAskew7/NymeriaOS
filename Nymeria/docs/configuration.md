...
| `LLM_REFUSAL_SWAP_MODE` | `off` | `off`, `ask`, `auto` | Whether an EMPTY provider refusal (the safety classifier ended the response before any text or tool call, e.g. Fable 5's `stop_reason="refusal"`) discards the refused response and re-runs the call on the next fallback model. `off` keeps the rewind-and-restore behavior. `ask` pauses a consent-capable interactive turn (timeout auto-swaps); autonomous turns and non-interactive channels auto-swap immediately. `auto` swaps silently everywhere. Partial-output refusals are never swapped. Per-thread overridable. |
