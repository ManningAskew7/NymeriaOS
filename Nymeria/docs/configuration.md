...
| `TRIGGER_FAILURE_ALERT_COOLDOWN_MINUTES` | `180` | Minimum minutes between action-failure ALERTS for one trigger. A flapping trigger (fails, succeeds, fails) resets its streak every time and so never reaches the pause threshold, which without this would alert once per episode indefinitely. The PAUSE alert is never suppressed, and a resume clears the cooldown (0 disables; 0-10080) |
