...
| `EXEC_SANDBOX_ENABLED` | `true` | Confine the code the agent runs to a Landlock filesystem sandbox (Linux only; ignored where the kernel cannot enforce it, with one warning at first use). Covers the three surfaces that execute agent-supplied code: `bash_execute`, the Python custom-tool runner and the workflow runner. Other spawns (MCP stdio servers, the Claude Code bridge) are not confined yet and each says why at its call site. Subtracts rather than allowlists: a command reaches everything it did before except `/proc` file content (bar a few machine-wide files) and, on deployments where they sit outside the agent's working tree, the credential stores under the data dir. Closes the route by which any shell could read the credential vault's master key out of the process environment. The `.env` files are never denied (they sit in the project root by construction, which is the working tree). Costs about 0.1s per call, the process-table tools (`ps`, `top`; `pgrep`/`pkill` return empty rather than erroring), and anything needing privilege (`sudo`, `ping`, `mount`), since Landlock requires the kernel's `no_new_privs` flag. See `docs/agent-systems/tools.md`, `bash_execute`. |
