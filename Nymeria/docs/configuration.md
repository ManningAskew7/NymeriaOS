...
| `LLM_FALLBACK_MODELS` | No | `anthropic:claude-haiku-4-5-20251001` | Comma-separated ordered fallback models tried by the backend after primary retries are exhausted for a transient provider/transport error before output starts. Auth (401/403) and invalid-request (400) failures also advance the chain, immediately and only to an entry on a different provider (same-provider entries are skipped for those classes since they share the failing credential or wire). Entries use the active provider by default, or `provider:model-id` for any known provider in the LLM registry (so one provider's outage does not also disable the fallback). CLI shortcut: `/fallback`. |
...
| `LLM_EXTENDED_THINKING` | `false` | true/false | Enable extended thinking/reasoning for compatible models. Command shortcut on every surface: `/think on\|off\|low\|medium\|high\|xhigh\|max [global\|thread]` (aliases `/reasoning`, `/thinking`) sets both fields in one command, thread-scoped when a thread is active. `/fast` toggles the active thread between `LLM_MODEL` and `LLM_FAST_MODEL`; `/smart` does the same with `LLM_SMART_MODEL`. Both are also in the central command registry (available on desktop and bots), and `fast`/`smart`/`default` work as `llm_model` aliases in `spawn_thread`. |
...
| `ALLOW_UNBOUND_TOOL_CALLS` | `false` | Dynamic binding only. Let the model dispatch a tool by emitting a call for it even when the tool is not in its bound list, provided it passes the same deferred gates as `tool_invoke` (management denylist, role gates, `disabled_tools`). Also drops the resident `tool_invoke` tool from the bound schema to save tokens. Off by default: an unbound call is refused with an error that names the kit whose TTL lapsed (when that is why the tool is gone) and only the remedies this thread can reach (`Skill(name=<kit>, ttl=...)` or the user's `/kit`, `tool_manage` when bound, `tool_invoke` when bound and the tool is not a protected management tool). Only enable on providers that reliably emit calls for tools not present in the schema. |
...
| `ACCOUNT_SERVER_BROWSER_TOKEN_TTL_DAYS` | `3650` | Lifetime for the token baked into the server browser, which is exempt from `ACCOUNT_TOKEN_TTL_DAYS`. Long on purpose: it is a machine credential with no renewal path and no human in the loop, so the ordinary lifetime would silently break the browser tools on every install. Re-running `nymeria browser configure` revokes and re-mints it at any time. See [deployment/server-browser.md](deployment/server-browser.md) |
...
| `SERVICE_TOKEN_WARN_DAYS` | `14` | Warn admins (notification center + push) when a service-shaped account token (the `bot-service` user, or any token whose label contains `service`) is within this many days of expiry. Hourly API-process sweep, both shapes; each token warns once per threshold (window entry, 3 days, 1 day, expired). `0` disables. See `docs/accounts.md`. |
...
| `NYMERIA_PUBLIC_URL` | - | Browser-reachable public API origin used to build one-time credential setup links for chat bots and OAuth authorization-code redirects, e.g. `https://nymeria.example.com`. Settable at runtime, no restart: `/env set NYMERIA_PUBLIC_URL <url>` (or `PATCH /settings`), listed in `/env show`; setting it warns on values that look like email-rewriter wrappers (Outlook Safe Links, urldefense). Desktop modal prompts still work when unset; chat bots will report that a public URL is required. OAuth `request_credential` calls return `status="missing_public_url"` when unset unless the agent retries with `use_localhost=True` (only safe when the user's browser is on the same machine) or the provider supports device-code (currently Outlook). |
| `NYMERIA_ERROR_REPORT_EMAIL` | - | Destination for the desktop "Report problem" button (`POST /report`). The report (thread ID, last 10 messages, client info, optional description) is emailed here via the Outlook email tool, sent from your connected Outlook account. Use a dedicated inbox, not a personal one, since reports carry other users' message content. If unset, `/report` returns 503 and emails no one. With several Outlook accounts connected, set `OUTLOOK_DEFAULT_ACCOUNT_ID` (the report runs outside any thread, so no thread binding applies and the send otherwise fails closed). |
| `HARNESS_REPORT_DIR` | - | Existing directory the optional `harness_report` catalog tool drops agent-filed issue/friction reports into, one new markdown file per report, for operator triage. The tool only ever creates new files (exclusive create, sanitized names) and refuses honestly when this is unset or the directory does not exist; it never creates the directory, so a missing Docker bind mount surfaces instead of reports landing on the container's ephemeral layer. |
...
| `NYMERIA_MCP_CHAT_WAIT_SECONDS` | derives from `TOOL_TIMEOUT` | Default seconds `nymeria_chat` waits on `mode="ask"` before returning a partial transcript plus a resume token. Unset derives from `TOOL_TIMEOUT`. Per-call `wait_seconds` overrides it (1-3600). This is the MCP client's own ask/handoff model, not the in-process one: an MCP client is a user rather than a thread and cannot be woken, so thread-to-thread calls (now requests, bounded by `CALLABLE_WAIT_MAX_SECONDS`) do not apply here. **Your MCP client's transport timeout must exceed whichever value is in play**, or the client aborts first and you get a transport error instead of the partial; the repo `.mcp.json` is set above the 3600s ceiling for this reason. An out-of-range value here is clamped, while an out-of-range `wait_seconds` argument is refused. |
...
| `SERVER_BROWSER_HOME` | unset | Home directory of this install's server browser (the headless Chrome running the Nymeria extension beside the backend). Written by `nymeria init`; `nymeria browser` reads it to find the rig, falling back to the pointer `nymeria browser configure --home` leaves in `<project root>/data/server-browser-home`, then to `<project root>/data/server-browser`. The PROCESS environment wins over the env files here, which is a trap on a host running two installs (see the doc). Its PRESENCE is the signal that matters: it tells the `chrome_*` tools that the rig's own account (the bootstrap admin) has a server browser, so a no-browser refusal points at `nymeria browser status` instead of an extension popup the server browser does not have. Other accounts are answered by their own roster, so a household member on a shared instance still gets the "install the extension in your own Chrome" advice, which is the only route they have. Setup retires the key when a reconfigure finds no rig; delete the line by hand if you remove a rig without re-running setup. Set it to move the rig off the data dir; snapshots exclude the rig by this resolved path as well as by name, so a rig moved inside the data dir stays out of the artifact. See [deployment/server-browser.md](deployment/server-browser.md) |
...
| `OUTLOOK_DEFAULT_ACCOUNT_ID` | - | Default Outlook account for email tools when several are connected and the calling thread has no binding (`credentials.md`, "Which account a call uses"). Ignored when a thread binding has narrowed the view to other accounts; with several accounts, no binding, and no default, tools fail closed and list the accounts instead of picking the first |
...
| `INTERACTIVE_ADMISSION_WAIT_SECONDS` | `10` | Seconds an over-capacity interactive chat request may wait for a free turn slot before the `429` (0-120; `0` = reject immediately). Only meaningful when `MAX_CONCURRENT_INTERACTIVE` > 0. Runtime limit changes apply to NEW admissions only: raising (or disabling) `MAX_CONCURRENT_INTERACTIVE` does not wake requests already waiting at the old ceiling (they are woken only by a finishing turn's slot handoff, and otherwise shed at the end of their bounded wait, reported against the limit they arrived under), while new arrivals are admitted under the new limit immediately. |
...
| `TOOL_TIMEOUT` | `300` | Max seconds a single tool call may run before the tool node terminates it (30-900). One exception: a call that DECLARES an inline wait (a callable thread tool's `wait_seconds`, `wait_for_reply`'s `timeout_seconds`, `spawn_thread`'s `wait_seconds`) is killed at its own clamped wait plus 60s instead, so a declared wait is neither cut short nor extended by this value. Every other tool, `bash_execute` included, is bounded here. |
| `CALLABLE_WAIT_MAX_SECONDS` | `600` | Per-call ceiling on an inline wait for another thread's reply (30-21600), applied to a callable thread tool's `wait_seconds`, `wait_for_reply`'s `timeout_seconds`, and `spawn_thread`'s `wait_seconds`. Waits are chainable, so this bounds ONE call rather than the total time a caller may spend waiting, and it is independent of `TOOL_TIMEOUT` (the tool-node kill for a waiting call derives from the clamped wait, not from the tool timeout). Thread-to-thread calls are requests: see `docs/agent-systems/tools.md`, "Callable Thread Tools (Dynamic)". |
...
| `EXEC_SANDBOX_ENABLED` | `true` | Confine the code the agent runs to a Landlock filesystem sandbox (Linux only; ignored where the kernel cannot enforce it, with one warning at first use). Every agent-reachable spawn is confined except one, and the exception is the operator-relevant part: Claude Code itself aborts under the `/proc` denial the policy always applies, so a `claude_code` run has the reach of the account its runner runs as. (The rule the build enforces, rather than a list that ages: a spawn under `nymeria/core/` or `nymeria/tools/` is either confined or declares at its call site why not, and `tests/test_exec_sandbox_gate.py` fails on one that does neither.) Subtracts rather than allowlists: a command reaches everything it did before except `/proc` file content (bar a few machine-wide files) and, on deployments where they sit outside the agent's working tree, the credential stores under the data dir. Closes the route by which any shell could read the credential vault's master key out of the process environment. The `.env` files are never denied (they sit in the project root by construction, which is the working tree). Costs about 0.1s per call, the process-table tools (`ps`, `top`; `pgrep`/`pkill` return empty rather than erroring), and anything needing privilege (`sudo`, `ping`, `mount`), since Landlock requires the kernel's `no_new_privs` flag. That last cost now reaches MCP stdio servers too: one needing a setuid helper (a browser-automation server falling back to Chrome's `chrome-sandbox`) will not start, and the fix is `EXEC_SANDBOX_ENABLED=false` or a server that does not need one. One more edge, specific to deployments whose data dir sits OUTSIDE the project tree: denying a store makes its parent unable to hand out read access to files created later, so a server writing a brand-new file DIRECTLY into the data dir cannot read it back until it reconnects (measured with an MCP memory server pointed there). Only direct children are affected, anything deeper under an existing directory is fine, and the policy is rebuilt per spawn so a restart clears it. See `docs/agent-systems/tools.md`, `bash_execute`. |
...
| `TRIGGER_FAILURE_PAUSE_AFTER` | `5` | Consecutive failed ACTION fires before a trigger is auto-paused, stopping its polling until `/triggers resume <id>` (a paused trigger's webhook endpoint also refuses with 409). Leaves `enabled` untouched; source-check failures never auto-pause; 0 disables auto-pause but does not release triggers already paused (0-1000) |
| `TRIGGER_FAILURE_ALERT_COOLDOWN_MINUTES` | `180` | Minimum minutes between failure ALERTS for one trigger, on BOTH planes. Action plane: a flapping trigger (fails, succeeds, fails) resets its streak every time and so never reaches the pause threshold, which without this would alert once per episode indefinitely. Source plane: this doubles as the REPEAT interval, so a permanently dead source re-alerts this often instead of going silent forever. The PAUSE alert is never suppressed, and a resume clears the cooldown (0 disables both, restoring one alert per episode; 0-10080) |
...
| `data/server-browser/` | The server browser's rig: Chrome for Testing, the staged extension with its baked token (mode 0600), and `profile/` (mode 0700), which holds every site session a human signed it into. Treat `profile/` like a private key. Snapshots never capture it (an explicit exclusion, by name and by resolved home, no opt-in), and a restore leaves an existing rig in place rather than sweeping it aside, since the artifact has nothing to put back. Only present when the server browser is installed |
...
# EXEC_SANDBOX_ENABLED=true       # Landlock filesystem sandbox for agent-run commands (Linux; denies /proc content and the credential stores)
