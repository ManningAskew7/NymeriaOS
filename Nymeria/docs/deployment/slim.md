...
`python run.py slim` runs everything Nymeria needs for local/personal use
...
- `data/BOOTSTRAP_TOKEN.txt` is written (mode `0600`). Paste this `nym_...`
...
- `data/SLIM_SERVICE_TOKEN.txt` is written (mode `0600`). This is an
  *internal* admin token that the in-process MCP, watchdog, trigger-fire
  ...
  same process. **Do not** paste this into the Setup Wizard — it's a
...
`python run.py slim` applies the following runtime env overrides **before**
...
| `--data-dir` | — | Runtime data directory (writes `NYMERIA_DATA_DIR`) |
