...
| `components/chat/AgentActivityIndicator.svelte` | Inline animated phase text for active assistant streams (`Processing...`, `Thinking...`, `Typing...`, `Formulating...`, `Processing results...`, `Waiting...`), driven by SSE activity state including tool-argument deltas and an initial silence fallback | Desktop-only v1; mobile still uses its existing dots/ThinkingIndicator flow |
