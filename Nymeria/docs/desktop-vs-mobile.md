...
| `components/chat/AgentActivityIndicator.svelte` | Inline animated phase text for active assistant streams (`Processing...`, `Thinking...`, `Typing...`, `Formulating tool calls...`, `Processing results...`, `Waiting...`), driven by SSE activity state including tool-argument deltas | Desktop-only v1; mobile still uses its existing dots/ThinkingIndicator flow |
