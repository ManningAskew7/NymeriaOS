...
1. **Eager claim from the desktop.** `threadsStore.createThread()` (`nymeria-desktop/src/lib/stores/threads.svelte.ts`) generates a UUID and immediately fires `POST /threads/{thread_id}/claim` (fire-and-forget) so the backend has an ownership row before any chat-app binding can route a message into the thread. Without this, an admin who created a UUID, bound it to a Telegram chat, and let the bound user send the first message would silently transfer ownership to that user.
