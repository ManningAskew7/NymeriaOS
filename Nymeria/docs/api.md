...
| `ui_prompt` | Interactive HTML form from the `ui_prompt` tool; the agent blocks until it resolves. The desktop app renders the agent-authored HTML in a sandboxed iframe modal (`sandbox="allow-scripts"`, null origin, CSP `connect-src 'none'`, all assets inlined) and POSTs the user's answer to `POST /ui-prompts/{prompt_id}/result` with `{"status": "submitted"\|"cancelled", "values": {...}}` (owner or admin; the first resolve wins, later POSTs get `delivered: false`). Other clients ignore the event and the tool times out | `prompt_id`, `title`, `html`, `timeout_seconds`, `expires_at` |
| `ui_prompt_result` | A `ui_prompt` was resolved: a client answered or dismissed it, or the backend closed it (tool timeout, thread abort, orphan sweep); every client retracts its copy of the modal. Deliberately never carries the submitted values | `prompt_id`, `status` (`submitted`/`cancelled`/`timeout`/`aborted`/`swept`) |
