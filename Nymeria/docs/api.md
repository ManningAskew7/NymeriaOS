...
data: {"type": "tool_call", "name": "web_search", "args": {...}, "description": "Search the web for current information.", "thread_id": "abc123"}
...
| `tool_call` | Tool being invoked | `name`, `args`, `description` |
...
The `tool_call` `description` is a short, display-only label derived from the
tool name (its registry metadata summary) for use in frontend tool cards. It is
re-derived on each render, including history sync, and is never persisted to the
conversation checkpoint or shown to the model. It may be `null` for tools
...
| `tool_call_delta` | Status-only hint that the model is streaming tool-call argument chunks before the tool starts | none |
| `tool_call` | Tool invocation | `id`, `name`, `args`, `description` |
...
data: {"type":"tool_call","id":"tool1","name":"bash_execute","args":{"command":"ls ~/inbox"},"description":"Execute a shell command and return the output."}
