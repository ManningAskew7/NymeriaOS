...
- Name collisions with built-in tools or other callables are rejected
...
`bash_execute`, `file_read`, and `file_write` are local system access tools.
They live in `OPTIONAL_TOOLS`, are admin-only, and must be enabled per-thread;
they are not loaded by default and are stripped from user-level default tool
sets during startup migration.

**Core tools by category:**

| Category | Tools |
|----------|-------|
| Core System | web_search, consult |
