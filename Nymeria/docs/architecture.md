...
Any thread with `callable=True` in its thread config becomes a directly invocable tool. This replaces the old sub-agent registry system.
...
**Built-in callable threads:** SelfModifyAgent, OutlookAgent, CalendarAgent, BrowserAgent (pre-configured via thread config)
...
|------|-------|-------------|
| **Core (`ALL_TOOLS`)** | 16 | Static tools always loaded |
| **Callable Thread Tools** | Dynamic | One per callable thread, generated by `tool_factory.py` |
| **Optional (`OPTIONAL_TOOLS`)** | ~50 | Per-thread enabling (13 Outlook + 4 trigger + 9 browser + 14 calendar + 7 self-modify + 2 utility) |
...
| Agent Management | clear_agent_context, reload_all |
| Notification | notify (unified Telegram/Discord/Slack) |
