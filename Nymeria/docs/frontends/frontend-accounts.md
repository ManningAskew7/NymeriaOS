...
```
nymeria-desktop/src/lib/components/account/   - account UI (mobile mirrors most files)
...
├── AccountMenu.svelte           popover/sheet for account actions
...
nymeria-{desktop,mobile}/src/lib/services/api/
├── base.ts                      probeConnection(), getHeaders(), _toastAndExtractError()
├── accounts.ts                  /me, /me/tokens, /admin/users, platform, chat-app, BYO bot methods
└── index.ts                     NymeriaAPI singleton exported through api.svelte.ts
```

Mobile lives at `nymeria-mobile/src/lib/...` and mirrors the core account files,
but it is single-connection. `AccountSwitcher.svelte`,
`AddAccountSheet.svelte`, and `connections.svelte.ts` are desktop-only.
Mobile `AccountMenu.svelte` opens Connection settings instead of separate
Switch/Add account surfaces, and mobile token dialogs do not include
desktop-only save-to-switcher actions.
...
`Sidebar.svelte` (desktop) renders `AccountBadge` in the footer and collapsed
sidebar. On mobile, `LeftPanel.svelte` renders it as a 40x40 button alongside
Settings and Notifications. Shows:

- Deterministic-colour avatar (initials hashed from `user_id`).
- Desktop full mode shows display name (or email fallback) plus a secondary
  email or active-connection verification error.
- Collapsed desktop and mobile render the avatar only.
- Avatar state is `loading`, `connected`, or `unverified`; disabled styling is
  used in switcher/user rows, not on the badge itself.
...
| Switch account | Desktop only. Opens `AccountSwitcher`. |
| Add account | Desktop only. Opens `AddAccountSheet`. |
| Connection settings | Mobile only. Opens SettingsPanel -> Connection tab. |
| Sign out | Desktop clears active saved connection and signs out; mobile signs out. |

Desktop state is lifted up to `AccountBadge` so the badge can coordinate menu,
switcher, and add-sheet visibility. Mobile `AccountBadge` only owns the bottom
sheet open state.
...
- Linked platforms (admin only - `<PlatformLinkingSection userId={identity.id} />`)
...
On boot the active entry is auto-verified (`connectionsStore.verifyEntry(id)` runs in a 500ms-deferred `setTimeout` at the bottom of `connections.svelte.ts`). Each row in the switcher shows the cached identity; the per-row more-actions menu can re-hit `/me` against that entry's URL+token.
...
The raw token only ever lives in the issuing component's local state. Desktop
admin-token dialogs can save that raw token into the saved-account switcher
while the dialog is still open; otherwise closing the dialog drops it forever.
Mobile has no saved-account switcher, so mobile copy-once dialogs only copy and
close. This matches the backend contract (`accounts.md::Tokens` - only sha256
is stored).
...
`services/api/base.ts::_toastAndExtractError()` is the shared chokepoint for
account/admin failures. Account methods live in `services/api/accounts.ts` and
call it before throwing on every non-2xx response:

| Status | Toast kind | Side effect |
|---|---|---|
| 401 | `auth_invalid` | `errorsStore.pushAuthInvalid()` signs out; desktop also clears the active saved connection. |
...
Desktop owns the `showMenu` / `showSwitcher` / `showAddAccount` state and
renders `AccountMenu`, `AccountSwitcher`, and `AddAccountSheet` as siblings
inside `.account-wrapper` so absolute-positioned popovers anchor correctly.
Two render modes (full vs collapsed-icon) mirror the existing sidebar footer
patterns. Mobile only owns `showMenu` and renders a single avatar button.

### `AccountMenu.svelte`

Reads `configStore.identity` and `configStore.apiUrl` reactively. Desktop binds
click-outside / Escape close handlers via `$effect`; mobile renders a bottom
sheet with backdrop click handling. `Manage users` is gated on
`identity?.role === 'admin'`. Desktop exposes Switch account and Add account;
mobile exposes Connection settings instead.
...
|---|---|---|---|---|
| `self` | `GET /me/tokens` | `POST /me/tokens` | `DELETE /me/tokens/{token_hash_prefix}` | (hidden) |
| `admin` | `GET /admin/users/{user_id}/tokens` | `POST /admin/users/{user_id}/tokens` | `DELETE /admin/users/{user_id}/tokens/{token_hash_prefix}` | `POST /admin/users/{user_id}/tokens/rotate` |

`load()` is wrapped in `untrack()` inside the `$effect` so its internal
`if (loading) return` re-entry guard doesn't establish `loading` as a tracked
dependency of the effect (which would race itself on every fetch toggle). Issue
button disabled state is tied to `issuing`, **not** `loading` - list-load
delays must never block issuing. Desktop admin mode also passes the optional
save-account actions into `CopyOnceTokenDialog`; mobile omits those actions.

### `PlatformLinkingSection.svelte`

Always uses the admin endpoints (`GET / POST / DELETE /admin/users/{user_id}/platforms`). For self use in `AccountTab` this still works because the section is only rendered for admins viewing themselves. The provider dropdown is hardcoded to the backend chat-app provider union (`discord`, `telegram`, Slack/WhatsApp/Teams). Provider ID input is monospace because these are platform-native IDs.
...
Desktop has optional `onSaveAccount` / `onSaveAndSwitch` props for admin-issued
tokens while the raw token is still available. Mobile does not implement those
props.
...
2. **bind** - Calls `api.issueChatAppBindCode(threadId, 'telegram')`. Same UX: code + deep link. Polls `/threads/{thread_id}/chatapp/bindings` every 2s; auto-advances when the binding row appears.
...
The public singleton is exported from `services/api.svelte.ts`, assembled in
`services/api/index.ts`, and implemented by the class chain under
`services/api/`. Account/admin methods live in `services/api/accounts.ts`; the
shared auth headers, `probeConnection()`, `resolveUserId()`, and
`_toastAndExtractError()` live in `services/api/base.ts`. Account methods share
this shape:
...
If you add a new account/admin endpoint, follow this pattern in both apps so
error toasts stay consistent. If you need a 409 handled with a new kind, extend
`_toastAndExtractError`'s body-substring check (the matchers are intentionally
loose; the backend is the source of truth for the message text).
...
| Badge shows "Not signed in", "Connecting...", or a verification error | `routes/+page.svelte`, `config.svelte.ts::refreshIdentity`, and desktop `connections.svelte.ts::verifyEntry`. | The badge itself uses loading/connected/unverified avatar states. Desktop switcher rows use the disabled avatar state when a saved entry fails verification. |
...
3. **Add API method**: in both apps' `services/api/accounts.ts` (or the closest domain file under `services/api/`), add a method that follows the existing `_toastAndExtractError` pattern.
...
3. **Identity surface** - desktop sidebar bottom shows your avatar and name; mobile shows the avatar button. Click -> AccountMenu lists Manage account and Manage users for admins. Desktop also lists Switch account and Add account; mobile lists Connection settings.
...
7. **Switch to non-admin** - on desktop, Account menu -> Switch account -> click the new entry. Verify the Users tab is hidden, MCP install button shows "Admin only", and `self_modify` tool has the admin-only badge.
...
11. **Mobile parity** - repeat 3, 4, 5, and 6 on mobile. Note that mobile has no AccountSwitcher / AddAccountSheet; sign into the created user by updating Settings -> Connection or by signing out and using SetupWizard with the copied token.
