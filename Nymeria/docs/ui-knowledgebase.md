...
**Activity labels:** `Processing...`, `Thinking...`, `Formulating...`, `Waiting...`, `Processing results...`. Full-screen mode shows the live activity segment in both the active `Nymeria` transcript separator and the status bar before response text starts streaming. Rich REPL uses the same status formatter in the live line above the prompt while the prompt is active. The CLI does not put raw thinking text in the status bar. Quiet processing/thinking transitions become `Formulating...` after about one second. Response typing hides the transcript activity indicator.
