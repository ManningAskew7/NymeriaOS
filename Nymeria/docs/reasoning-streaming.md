...
| Ollama native (`provider="ollama-native"`) | `langchain-ollama` `/api/chat` | `reasoning_content` in `additional_kwargs`, native `<think>` rescue | ✅ yes; `reasoning` toggle (bool or low/medium/high for gpt-oss) wired automatically when extended thinking is enabled |
