...
Humans see what the model saw: `/history` strips a next-turn notice from the user bubble and re-emits it as a `system` entry with `kind: "tool_expiry_notice"`, before the prompt it rode on. A mid-turn notice is an attributed system input in the expanded `queued_batch` block, alongside any requests sent in the same continuation. It remains visible regardless of `show_autonomous_prompts`. Live clients receive the same batch and exact model content through `prompt_injected`.
