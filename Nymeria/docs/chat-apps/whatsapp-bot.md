...
and Discord use) and the command's markdown result is sent back in chat, so
...
(`surface="whatsapp"`). Commands that execute as chat turns (`/skill`,
`/kit`) fall through to the normal chat path automatically. The local
commands above are matched first (with or without a leading `/`).
