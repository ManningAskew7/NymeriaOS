...
- **Provider management**  -  `/provider`, `/provider list`, `/provider set <provider> api_key=<key>`, `/provider test [provider]`, and `/provider switch <provider>` manage LLM provider auth on every surface via the central command registry. Secrets are written straight to the backend settings API (set/test/switch are admin-gated); credential status comes from the server environment listing.
