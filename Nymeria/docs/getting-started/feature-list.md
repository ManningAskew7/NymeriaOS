...
- **Guidance**  -  `self-improve` is a text-only guidance skill (binds no tools) holding the operating philosophy and a router to the focused, default-on management kits below (the default-on set also includes `trigger-management`, `hook-management`, and `callable-thread-builder`)
...
- **Model hot-switching**  -  Change model per-thread or globally at runtime without restart. `/model <name>` checks the name against the provider's live model list first and refuses an off-list id with near-matches; `--force` writes it anyway, and a provider that exposes no list writes without the check
...
| **Capability Expansion** | `Skill(name="self-improve", ttl="1h")` is text-only guidance (binds no tools) and routes to its four capability kits, default-on alongside `trigger-management`, `hook-management`, and `callable-thread-builder`, each binding its tools only when activated: `tool-management` (`tool_search`, `tool_manage`, `tool_create`, `api_discover`, `http_request`), `skill-management` (`skill_manage`, `skill_write`, `skill_edit`), `mcp-management` (`manage_mcp`), and `credential-management` (`auth_inspect`, `auth_cleanup`, `auth_bindings`, `request_credential`) |

The code-owned source of truth is `nymeria/tools/__init__.py`: ~16 seed tools in
`SEED_TOOLS` (the default for new threads) and roughly 1,260 catalog tools in
`CATALOG_TOOLS` (both counts drift as the seed set is tuned and integrations
land; read `SEED_TOOLS` and regenerate `tools-index.md` for the live totals).
...
- **Model fallback chain**  -  `/fallback`, `/fallback add <model-id> [--position N]`, `/fallback remove <model-id>`, `/fallback set <model-id ...>`, and `/fallback clear` manage `LLM_FALLBACK_MODELS`. The backend tries the ordered chain for retryable provider/transport failures before any output chunk, and jumps straight to a different-provider entry on auth (401/403) or invalid-request (400) failures, so every frontend surface benefits from the same resilience behavior.
- **Fast / smart model tiers**  -  `/fast` and `/smart` (with `on`/`off`) switch the active thread between the default model and `LLM_FAST_MODEL` / `LLM_SMART_MODEL`; `/fast set <model-id>` (and `/smart set`) store the tier value. Status bar shows `FAST` while fast is active. Each value may be a model id or `provider:model` for a different provider.
