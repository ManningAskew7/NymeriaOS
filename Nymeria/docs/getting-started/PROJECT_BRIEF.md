...
- **Seed tools** (`SEED_TOOLS`) - the small always-loaded core: shell, files,
  web, memory/RAG, TODOs, notifications, slash commands, credential prompting
- **Dynamic callable thread tools** - one per callable thread, generated by
  `tool_factory.py`
- **Catalog tools** (`CATALOG_TOOLS`, derived from the `registry.py`
  tool-group registry, never hand-listed) - the large per-thread optional
  catalog: browser, Outlook, Google, Microsoft Graph, triggers, image
  generation, service integrations, self-modify, and related families

Counts are deliberately not stated here (they change; a stale count in this
doc has misled before). The `nymeria/tools/__init__.py` module docstring and
`../agent-systems/tools.md` are the source of truth.
