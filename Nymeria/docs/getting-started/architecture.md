...
**Modelling a deliberately idle TODO:** the two filters above are the supported exemptions, and they are the answer for a TODO that is waiting rather than wedged. A `recurrence`, or a `scheduled_for` in the future, takes the TODO out of the staleness check entirely. (Pair them: set `recurrence` together with a future `scheduled_for`. The ticker only fires from the schedule DB, so a recurrence without a schedule never fires; it only silences the watchdog.) Nothing else does: every `nym_todo` write advances `updated_at`, so "touching" a TODO (a notes rewrite, re-setting the same status) only resets the clock and buys one more alert. A long-lived watcher ("check the laundry every 45 minutes") is therefore best modelled as ONE self-rescheduling TODO that re-arms its own `scheduled_for` at the end of each run, not as an unscheduled parent plus a trail of scheduled children: the parent is stripped of its own exemption the moment it fires (`clear_todo_schedule` nulls `scheduled_for` while leaving it active), and nothing links children back to it.

**Repeat nudges back off:** the nth nudge for one TODO requires `todo_staleness_minutes * 2**n` of quiet since its last touch, so a TODO that keeps being touched without changing status is alerted at 1x, 2x, 4x and so on (capped at 64x), instead of once per sweep forever. The count resets when the TODO's STATUS changes, when it is completed, or when it disappears; an `updated_at` advance alone does not reset it, because that is what a no-op touch produces. Note that `todo_staleness_minutes`, `watchdog_interval_minutes` and `watchdog_enabled` are all captured at construction: changing them through `PATCH /settings` reports `restart_required` and needs a restart of the process that hosts the sweep (in Docker, the worker).

**State:** The sweep keeps per-(user, todo_id) "last nudge time", "last-seen updated_at", last-seen status and repeat-nudge count in memory. Nudge eligibility resets naturally on the next cycle whenever `updated_at > last_seen`. A nudge turn that fails before producing any response retries next cycle, capped at 3 consecutive failures (then suppressed until the TODO is updated). State is lost on restart, which is fine: stale TODOs will simply re-nudge on the next cycle. Delivery counters surface in the worker heartbeat (`watchdog` key).
...
|------|-------|-------------|
| **Seed (`SEED_TOOLS`)** | Current code-defined set | Seeds each user's editable `default_thread_tools`; on by default but user/thread-overridable (see pipeline below) |
| **Callable Thread Tools** | Dynamic | One per callable thread, generated by `tool_factory.py` |
| **Catalog (`CATALOG_TOOLS`)** | Current code-defined set | Bindable catalog: enabled per-thread or promoted into `default_thread_tools` |
...
Credential tools (`auth_inspect`, `auth_cleanup`, `auth_bindings`, `request_credential`), `consult`, `personality_set`, and the `web_search_*` providers are no longer seeded; they are opt-in catalog tools (the credential tools are bound on demand by the default-on `credential-management` Skill Kit).
...
## Delegated Execution: `/orchestrate`

`/orchestrate` turns a regular thread into a long-running, structured-work
mode. It is a command-specific chat-stream lifecycle that binds an internal
Skill Kit as part of its setup, then rewrites the user's message into a
kickoff prompt the agent acts on.

| | `/orchestrate` (delegated) |
|---|---|
| User's thread X is the… | **orchestrator** |
| Helper thread is the… | **worker** (forked from X via `branch_thread`) |
| Owns the task list | X |
| Authority to mark "done" | X (self-managed) |
| Helper lifetime | `temporary` (idle-deletes) |
| Persisted state | None  -  agent self-manages via `nym_todo` |
...
- Workers are spawned via `spawn_thread(mode="branched",
  ...
  idle-delete when the objective is finished.
...
A second, supervised mode (`/goal`) once sat beside this one: a worker
thread that structurally could not mark its own tasks done, with the
...
| Chat-stream intercept | `nymeria/api/routers/chat.py` |
