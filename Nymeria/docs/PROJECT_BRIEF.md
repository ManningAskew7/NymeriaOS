...
- **Optional tools** enabled per thread (`OPTIONAL_TOOLS`) across local shell/file access, Outlook, browser, trigger, Twitch, self-modify, and related families

Core categories include web/consult, profile/RAG, tasking, notification, and runtime utility tools. Local shell/file access (`bash_execute`, `file_read`, `file_write`) is admin-only and per-thread optional. See `nymeria/tools/__init__.py` and `docs/tools.md` for the current source of truth.
...
Operational note:
- Local execution tools (`bash_execute`, `file_write`) are powerful by design; they are admin-only per-thread optional tools, and deployment hardening is still required for exposed environments.
