...
def _fetch_openrouter_models() -> Dict[str, Set[str]]:
    ...
    Also populates _context_limits_cache with context_length for each model.

    Returns:
        Dict mapping model_id to set of input modalities (e.g., {"image", "file", "text"})
    """
    global _context_limits_cache
    try:
        response = httpx.get(
        ...
        for model in data.get("data", []):
            ...
            if model_id:
                ...
                if context_length:
                    _context_limits_cache[model_id.lower()] = context_length

                # Extract modalities
                if input_modalities:
                    capabilities[model_id.lower()] = set(input_modalities)
