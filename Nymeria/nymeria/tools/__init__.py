...
from .ui_prompt import ui_prompt, UI_PROMPT_TOOLS
...
def _derive_public_exports() -> list[str]:
    """The canonical public export set for this package (backlog #51).

    ``__all__`` below is the materialized (sorted) output of this function: a
    fixed spine of public helpers plus every tool object and every ``*_TOOLS``
    group list bound at module level. It is written out as a plain literal, not
    ``__all__ = _derive_public_exports()``, so ruff and pyrefly can still see
    the re-exports statically (a computed ``__all__`` would re-flag every
    re-exported tool import as unused). ``tests/test_tool_catalog_registry.py``
    asserts the literal still equals this function's output, so a new tool that
    is not added to ``__all__`` fails CI instead of silently dropping off the
    export surface. Regenerate with ``sorted(_derive_public_exports())``.
    """
    spine = {
        "CATALOG_TOOLS",
...
# Generated from _derive_public_exports(); pinned by the golden test.
__all__ = [
    "ACTIVITY_FEED_TOOLS",
    "ADMIN_ONLY_TOOL_NAMES",
    "AUTH_MANAGER_TOOLS",
