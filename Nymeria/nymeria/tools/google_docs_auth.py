...
def google_docs_list_accounts(config: Annotated[RunnableConfig, InjectedToolArg] = None) -> str:
    ...
    if not rows:
        return "[Info]: No usable Google accounts authenticated for Docs. Use google_docs_auth_start to add an account."
    ...
    for row in rows:
        ...
        if not row["usable"] and row.get("reason"):
            lines.append(
                f"  Action: run `google_docs_auth_clear(account_id=\"{account_id}\")`, "
                "then `google_docs_auth_start`."
            )
