...
and the ``/provider cliproxy`` subscription-OAuth guidance. Handler methods
are resolved by ``CommandService.execute`` via
...
class ProviderSetupCommandsMixin:
    ...
    # ── CLIProxy subscription guidance ────────────────────────────────────
    ...
    async def _cmd_provider_cliproxy(
        ...
        """Subscription-OAuth guidance for the CLIProxy catalog (phase 1).

        Reports the target's route shape and the management-API status with
        guided next actions; the in-REPL OAuth chain over the existing
        ``/cliproxy/oauth/*`` routes is the filed phase 2.
        """
        from ..cliproxy.catalog import get_cliproxy_provider, list_cliproxy_providers

        if len(args) > 1:
            return "[Error]: Usage: /provider cliproxy [target]"
        ...
        if not args:
            ...
            for spec in list_cliproxy_providers():
                lines.append(f"  {spec.id:<12} {spec.label}")
            lines.append(
                "Management API: "
                + ("configured" if configured else "not configured")
            )
            lines.append("Details: /provider cliproxy <target>")
            return "[Info]: " + "\n".join(lines)

        spec = get_cliproxy_provider(args[0].strip().lower())
        if spec is None:
        ...
        if configured:
            lines.append(
                "Next: run `nymeria init` (subscription login branch) for a"
                " guided OAuth login and route apply, or use the desktop"
                " CLIProxy admin panel. An in-chat OAuth flow is planned."
            )
        else:
            missing = [
                name
    ...
    async def _setup_chain(
        ...
        *,
        ...
        lines = list(note_lines or []) + guidance
        text = "\n".join(lines) if lines else f"Configuring {spec.label}."
