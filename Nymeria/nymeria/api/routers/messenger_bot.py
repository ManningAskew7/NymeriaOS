...
_MESSENGER_SEEN_CACHE = _SeenMessageCache()
...
def _reject_stale_messenger_messages(payload: dict[str, Any]) -> None:
...
class InProcessMessengerAPI:
    """Nymeria API adapter used by the API-hosted Messenger webhook client."""
    ...
    async def _chat_stream(self, message: str, thread_id: str, user_id: str):
        ...
        try:
            ...
            logger.debug("Messenger done metadata failed", exc_info=True)
        try:
            ...
            logger.debug("Messenger auto-title failed", exc_info=True)
        yield done
    ...
    def _publish_platform_sync(self, thread_id: str, user_id: str) -> None:
        try:
            ...
            logger.debug("Messenger platform sync publish failed", exc_info=True)
...
def create_messenger_bot_router(
    ...
    async def receive_messenger_webhook(
        ...
        _reject_stale_messenger_messages(payload)
