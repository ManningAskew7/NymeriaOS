"""Meta Messenger webhook client for two-way Nymeria communication."""
...
class MessengerNymeriaAPI(Protocol):
    """Small API surface the Messenger client needs from Nymeria."""
...
def make_platform_user_id(sender_id: str) -> str:
    """Return the Messenger identity key stored in Nymeria platform links."""
    return normalize_psid(sender_id)


def make_platform_chat_id(sender_id: str, page_id: Optional[str] = None) -> str:
    """Return the Messenger chat key used by Nymeria chat-app bindings."""
    psid = normalize_psid(sender_id)
    page = _safe_id(page_id or "")
    if page != "unknown":
        return f"messenger:{page}:{psid}"
    return f"messenger:{psid}"


def make_thread_id(sender_id: str, page_id: Optional[str] = None) -> str:
    """Generate a native Nymeria thread ID for a Messenger direct chat."""
    psid = normalize_psid(sender_id)
    page = _safe_id(page_id or "")
    if page != "unknown":
...
@dataclass(frozen=True)
class MessengerReplyTarget:
    psid: str
    page_id: str


class _SeenMessageCache:
    """TTL cache for webhook message dedupe by Messenger event/message ID."""
...
class MessengerCommand:
    """Small parser for Messenger text commands handled before agent routing."""
    ...
    @classmethod
    def parse(cls, text: str) -> Optional["MessengerCommand"]:
        cleaned = text.strip()
...
def _extract_message_event(
    ...
    *,
    entry_page_id: str,
) -> Optional[MessengerInboundMessage]:
    ...
    if not raw_sender_id:
        return None
    sender_id = normalize_psid(raw_sender_id)
    page_id = _safe_id(str(recipient.get("id") or entry_page_id or ""))
    ...
    return MessengerInboundMessage(
...
def _extract_postback_event(
    ...
    *,
    ...
    if not message_id:
        message_id = f"postback:{page_id}:{sender_id}:{timestamp or 0}:{_safe_id(text)[:80]}"
    return MessengerInboundMessage(
        message_id=message_id,
        sender_id=sender_id,
        page_id=page_id,
...
def extract_inbound_messages(payload: Mapping[str, Any]) -> list[MessengerInboundMessage]:
    """Extract text-like inbound messages from a Messenger Platform webhook."""
    entries = payload.get("entry")
    if not isinstance(entries, list):
        return []
    out: list[MessengerInboundMessage] = []
    for entry in entries:
        if not isinstance(entry, Mapping):
            continue
        entry_page_id = _safe_id(str(entry.get("id") or ""))
        ...
        for event in events:
            if not isinstance(event, Mapping):
                continue
            inbound = _extract_message_event(event, entry_page_id=entry_page_id)
            if inbound is None:
                inbound = _extract_postback_event(event, entry_page_id=entry_page_id)
...
class MessengerGraphClient:
    """Minimal async Messenger Send API client."""
    ...
    def __init__(
        ...
        *,
        ...
        self.page_id = _safe_id(page_id or "")
    ...
    async def send_text(
        ...
        *,
        ...
        page = _safe_id(page_id or self.page_id)
        if page == "unknown":
            raise BotAPIError("MESSENGER_PAGE_ID is required to send Messenger replies")
        payload = {
            "recipient": {"id": normalize_psid(psid)},
            "messaging_type": messaging_type,
            "message": {"text": body},
        }
        response = await self._client.post(
            f"{self.base_url}/{page}/messages",
            headers={
...
class NymeriaMessengerBot:
    """Messenger Platform webhook processor."""

    def __init__(
        self,
        api: MessengerNymeriaAPI,
        messenger: MessengerGraphClient,
        *,
    ...
    async def refresh_bindings(self) -> None:
        try:
            entries = await self.api.list_chatapp_bindings(provider="messenger")
        except Exception:  # noqa: BLE001
            logger.exception("Failed to refresh Messenger chat-app bindings")
            return
    ...
    async def handle_message(self, message: MessengerInboundMessage) -> None:
        target = MessengerReplyTarget(psid=message.sender_id, page_id=message.page_id)
        command = MessengerCommand.parse(message.text)
        ...
        user_id = await self.api.resolve_platform_user("messenger", platform_user_id)
        if user_id is None:
            await self._reject_unlinked(target, platform_user_id)
            return

        thread_id = self._resolve_thread_id(message.sender_id, message.page_id)
        await self._stream_to_messenger(
    ...
    def _resolve_thread_id(self, sender_id: str, page_id: Optional[str]) -> str:
        chat_id = make_platform_chat_id(sender_id, page_id)
        return self._bindings.get(chat_id) or make_thread_id(sender_id, page_id)
    ...
    async def _handle_command(
        ...
        *,
        ...
        if command.name == "link":
            ...
            await self._cmd_bind(command.arg, message.sender_id, message.page_id, target)
        elif command.name == "unbind":
            await self._cmd_unbind(message.sender_id, message.page_id, target)
        elif command.name == "stop":
            await self._cmd_stop(message.sender_id, message.page_id, target)
    ...
    async def _cmd_link(
        ...
        await self._send_text(
            target,
            f"Linked this Messenger account to Nymeria user `{result.get('user_id')}`.",
        )
    ...
    async def _cmd_bind(
        ...
        await self._send_text(target, f"Bound this Messenger chat to `{thread_id}`.")
    ...
    async def _cmd_unbind(
        ...
        if not result.get("unbound"):
            await self._send_text(target, "This Messenger chat is not bound.")
            return
        self._bindings.pop(chat_id, None)
        self._binding_users.pop(chat_id, None)
        await self._send_text(
            target,
            "Unbound. Future messages will use the default Messenger thread.",
        )
    ...
    async def _cmd_stop(
        ...
        if user_id is None:
            await self._reject_unlinked(target, platform_user_id)
            return
        thread_id = self._resolve_thread_id(sender_id, page_id)
        try:
            await self.api.stop(thread_id, user_id=user_id)
        except BotAPIError as exc:
            await self._send_text(target, f"Couldn't stop the current run: {exc.detail}")
            return
        await self._send_text(target, "Stopped the current run for this Messenger chat.")
    ...
    async def _reject_unlinked(
        ...
        await self._send_text(
            target,
            "This Messenger account is not linked to a Nymeria user yet.\n"
            "Use `link <code>` here, or ask an admin to run "
            f"`python3 run.py users link-platform <email> messenger {platform_user_id}`.",
        )

    async def _stream_to_messenger(
        self,
        *,
        message: str,
        thread_id: str,
        ...
        handler = _MessengerStreamHandler(self, target)
        try:
            await consume_sse_stream(self.api.chat_stream(message, thread_id, user_id), handler)
        except Exception as exc:  # noqa: BLE001
            logger.exception("Messenger streaming failed; falling back to sync")
            try:
                ...
                logger.exception("Messenger sync fallback failed")
                await self._send_text(
                    target,
                    f"Sorry, I encountered an error: {fallback_exc or exc}",
                )

    async def _send_text(self, target: MessengerReplyTarget, content: str) -> None:
        for chunk in split_message(content, MESSENGER_TEXT_LIMIT):
            await self.messenger.send_text(target.psid, chunk, page_id=target.page_id)


class _MessengerStreamHandler:
    """Render Nymeria SSE events into Messenger Send API text messages."""

    def __init__(self, bot: NymeriaMessengerBot, target: MessengerReplyTarget) -> None:
        self._bot = bot
        self._target = target
        self._buffer = ""
        self._done = False
    ...
    async def on_response_chunk(self, content: str) -> None:
        ...
        if len(self._buffer) >= MESSENGER_TEXT_LIMIT:
