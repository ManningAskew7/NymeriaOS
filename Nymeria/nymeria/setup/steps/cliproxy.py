...
class CLIProxyLoginStep(WizardStep):
    ...
    async def _run_login(self) -> None:
        ...
        active = [
            entry
            for entry in files
            if str(entry.get("provider") or "").lower() == spec.auth_file_provider
            and not entry.get("disabled")
            and not entry.get("unavailable")
        ]
        if active and not self._force_relogin:
            account = str(active[0].get("account") or active[0].get("email") or "")
    ...
    async def _post_login(
        ...
        if spec.id == "claude":
            # Keep tool_prefix_disabled true on every Claude auth file: inert
            # on v7 proxies, required if one is rolled back to v6.9.36.
            try:
